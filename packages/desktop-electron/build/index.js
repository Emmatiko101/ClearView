...
require("./security");
const isDev = !electron_1.app.isPackaged; // dev mode if not packaged
process.env.lootCoreScript = isDev
    ...
    : path_1.default.resolve(__dirname, 'loot-core/lib-dist/electron/bundle.desktop.js'); // serve from build in production
...
async function createWindow() {
    ...
    win.on('unresponsive', () => {
        console.log('browser window went unresponsive (maybe because of a modal though)');
    });
    win.on('focus', async () => {
        if (clientWin) {
            const url = clientWin.webContents.getURL();
            if (url.includes('app://') || url.includes('localhost:')) {
                clientWin.webContents.executeJavaScript('window.__actionsForMenu.focused()');
            }
        }
    });
    ...
    if (process.platform === 'win32') {
        ...
        win.setMenu((0, menu_1.getMenu)(isDev, createWindow));
    }
    else {
        electron_1.Menu.setApplicationMenu((0, menu_1.getMenu)(isDev, createWindow));
    }
    clientWin = win;
}
...
function updateMenu(budgetId) {
    ...
    const menu = (0, menu_1.getMenu)(isDev, createWindow, budgetId);
...
electron_1.app.on('ready', async () => {
    ...
    electron_1.protocol.handle('app', request => {
        ...
        let filePath = path_1.default.normalize(`${__dirname}/client-build/index.html`); // default web path
        if (pathname.startsWith('/static')) {
            // static assets
            filePath = path_1.default.normalize(`${__dirname}/client-build${pathname}`);
...
electron_1.ipcMain.handle('save-file-dialog', async (_event, { title, defaultPath, fileContents }) => {
    ...
    return new Promise((resolve, reject) => {
        if (fileLocation) {
            fs_1.default.writeFile(fileLocation.filePath, fileContents, error => {
                return reject(error);
            });
        }
        resolve();
...
electron_1.ipcMain.on('screenshot', () => {
    if (isDev) {
        const width = 1100;
...
electron_1.ipcMain.on('update-menu', (_event, budgetId) => {
    updateMenu(budgetId);
});
electron_1.ipcMain.on('set-theme', (_event, theme) => {
    const obj = { theme };
    if (clientWin) {
        clientWin.webContents.executeJavaScript(`window.__actionsForMenu && window.__actionsForMenu.saveGlobalPrefs(${JSON.stringify(obj)})`);
    }
});
electron_1.ipcMain.handle('move-budget-directory', async (_event, currentBudgetDirectory, newDirectory) => {
    try {
        ...
        await (0, fs_extra_1.remove)(currentBudgetDirectory);
    }
    catch (error) {

...
function AppInner() {
  ...
  const maybeUpdate = async <T,>(cb?: () => T): Promise<T> => {
    if (global.ClearView.isUpdateReadyForDownload()) {
      dispatch(
        setAppState({
  ...
  async function init() {
    const socketName = await maybeUpdate(() => global.ClearView.getServerSocket());
  ...
  useEffect(() => {
    global.ClearView.updateAppMenu(budgetId);
  }, [budgetId]);

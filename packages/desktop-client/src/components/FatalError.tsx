...
function RenderSimple({ error }: RenderSimpleProps) {
  ...
  if ('IDBFailure' in error && error.IDBFailure) {
    ...
    msg = (
      <Text>
        <Trans>
          ...
          that ClearView requires to run. This might happen if you are in private
    ...
    msg = (
      <Text>
        <Trans>
          ClearView requires access to <code>SharedArrayBuffer</code> in order to
  ...
  return expanded ? (
    <>
      <Paragraph style={{ marginTop: 10 }}>
        <Trans>
          ClearView uses <code>SharedArrayBuffer</code> to allow usage from
      ...
      >
        ...
        />{' '}
        <Trans>
          I understand the risks, run ClearView in the unsupported fallback mode
        </Trans>
      </label>
      <Button
  ...
  return (
    <Modal name="fatal-error" isDismissable={false}>
      ...
      >
        ...
        <Paragraph>
          <Button onPress={() => window.ClearView?.relaunch()}>

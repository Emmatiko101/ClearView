...
export function ConfigServer() {
  ...
  const restartElectronServer = useCallback(() => {
    globalThis.window.ClearView.restartElectronServer();
    setError(null);
  }, []);
  ...
  function getErrorMessage(error: string) {
    switch (error) {
      ...
      default:
        return t(
          'Server does not look like an ClearView server. Is it set up correctly?',
        );
    }
  }
  ...
  async function onSelectSelfSignedCertificate() {
    const selfSignedCertificateLocation = await window.ClearView?.openFileDialog({
      properties: ['openFile'],
      filters: [
        {
      ...
      >
        {currentUrl ? (
          <Trans>
            ...
            server. We will validate that ClearView is running at this URL.
          </Trans>
        ) : (
          <Trans>
            ...
            validate that ClearView is running at this URL.
          </Trans>
        )}
      </Text>

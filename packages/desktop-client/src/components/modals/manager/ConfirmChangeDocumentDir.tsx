...
export function ConfirmChangeDocumentDirModal({
  ...
  const moveDirectory = async (close: () => void) => {
    ...
    try {
      if (moveFiles) {
        await globalThis.window.ClearView?.moveBudgetDirectory(
          currentBudgetDirectory,
          newDirectory,
        );
      }
      ...
      dispatch(
        addNotification({
          type: 'message',
          message: t('ClearView’s data directory successfully changed.'),
        }),
      );
      close();
    } catch (error) {
        ...
        <>
          ...
          >
            ...
            >
              <Text>
                <Trans>
                  You are about to change ClearView’s data directory from:
                </Trans>
              </Text>

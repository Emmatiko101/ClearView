...
function getErrorMessage(error: string): string {
  switch (error) {
    ...
    case 'not-zip-file':
      return 'This file is not valid. Please select an unencrypted archive of ClearView data.';
    case 'invalid-zip-file':
      return 'This archive is not a valid ClearView export file.';
...
export function ImportActualModal() {
  ...
  async function onImport() {
    const res = await window.ClearView?.openFileDialog({
      properties: ['openFile'],
  ...
  return (
    <Modal name="import-actual" containerProps={{ style: { width: 400 } }}>
      {({ state: { close } }) => (
        <>
          <ModalHeader
            title={t('Import from ClearView export')}
          ...
          <View style={{ ...styles.smallText, lineHeight: 1.5, marginTop: 20 }}>
            ...
            <View style={{ '& > div': { lineHeight: '1.7em' } }}>
              <Paragraph>
                <Trans>
                  You can import data from another ClearView account or instance.

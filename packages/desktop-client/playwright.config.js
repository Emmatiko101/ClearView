...
expect.extend({
  async toMatchThemeScreenshots(locator) {
    ...
    // Check lightmode
    await locator.evaluate(() => window.ClearView.setTheme('auto'));
    ...
    await locator.evaluate(() => window.ClearView.setTheme('dark'));
    ...
    await locator.evaluate(() => window.ClearView.setTheme('midnight'));
